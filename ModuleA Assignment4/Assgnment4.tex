...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Shard Node Server}: \texttt{app/sharding/node/server.js}
    \item \textbf{Docker Orchestration}: \texttt{app/docker-compose.yml}
    ...
    \item \textbf{Vite HMR Config Patch}: \texttt{app/vite.config.ts} (\texttt{ignored: ['**/sharding/**']})
    \item \textbf{Shard Data Volumes}: \texttt{app/sharding/data/shard\{0,1,2\}/shard.db}
    \item \textbf{Execution Commands}:
        \begin{itemize}
            \item Start cluster: \texttt{docker-compose up --build -d}
